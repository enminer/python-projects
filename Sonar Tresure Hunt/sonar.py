...
def drawBoard(board):
    tensDigitsLines = '   '
    for i in range(1, 6):
        tensDigitsLines += (' ' * 9) + str(i)

    print(tensDigitsLines)
    print('  ' + '0123456789' * 6)
    print()
...
def makeMove(board, chests, x, y):
    ...
    if smallestDistance == 0:
        ...
        if smallestDistance < 10:
            ...
            board[x][y] = 'X'
            return 'Sonar did not detect anything. All treasures out of range'
...
def enterPlayerMove(previousMoves):
    ...
    while True:
        ...
        move = move.split()
        if len(move) == 2 and move[0].isdigit() and move[1].isdigit(1) and isOnBoard(int(move[0]), int(move[1])):
            if [int(move[0]), int(move[1])] in previousMoves:
                print('You already moved their.')
                continue
...
Enter the coordinates to drop a sonar device. The ocean map will be
    marked with
how far away the nearest chest is, or an X if it is beyond then sonar
    device's
range. For example, the C marks are where chests are. The sonar device
    shows a
3 because the closest chest is three spaces away.
...
while True:
    ...
    while sonarDevices > 0:
        ...
        if sonarDevices == 0:
            ...
            print('for home with the chests still out there! Game over.')
